<template>
  <div class="app-root">
    <a
      class="github-corner"
      href="https://github.com/leepule/vue-canvas-sheet"
      target="_blank"
      rel="noopener noreferrer"
      aria-label="View source on GitHub"
    >
      <svg width="80" height="80" viewBox="0 0 250 250" aria-hidden="true">
        <path d="M0,0 L115,115 L130,115 L142,142 L250,250 L250,0 Z" class="octo-bg"></path>
        <path
          d="M128.3,109.0 C113.8,99.7 119.0,89.6 119.0,89.6 C122.0,82.7 120.5,78.6 120.5,78.6 C119.2,72.0 123.4,76.3 123.4,76.3 C127.3,80.9 125.5,87.3 125.5,87.3 C122.9,97.6 130.6,101.9 134.4,103.2"
          fill="currentColor"
          style="transform-origin: 130px 106px;"
          class="octo-arm"
        ></path>
        <path
          d="M115.0,115.0 C114.9,115.1 118.7,116.5 119.8,115.4 L133.7,101.6 C136.9,99.2 139.9,98.4 142.2,98.6 C133.8,88.0 127.5,74.4 143.8,58.0 C148.5,53.4 154.0,51.2 159.7,51.0 C160.3,49.4 163.2,43.6 171.4,40.1 C171.4,40.1 176.1,42.5 178.8,56.2 C183.1,58.6 187.2,61.8 190.9,65.4 C194.5,69.0 197.7,73.2 200.1,77.6 C213.8,80.2 216.3,84.9 216.3,84.9 C212.7,93.1 206.9,96.0 205.4,96.6 C205.1,102.4 203.0,107.8 198.3,112.5 C181.9,128.9 168.3,122.5 157.7,114.1 C157.9,116.9 156.7,120.9 152.7,124.9 L141.0,136.5 C139.8,137.7 141.6,141.9 141.8,141.8 Z"
          fill="currentColor"
          class="octo-body"
        ></path>
      </svg>
    </a>
    <nav class="demo-nav">
      <span class="nav-title">Demo 选择：</span>
      <button :class="['nav-btn', { active: currentRoute === 'basic' }]" @click="currentRoute = 'basic'"> 基础使用 </button>
      <button :class="['nav-btn', { active: currentRoute === 'random' }]" @click="currentRoute = 'random'"> 随机样式
      </button>
      <button :class="['nav-btn', { active: currentRoute === 'collab' }]" @click="currentRoute = 'collab'"> 实时协同
      </button>
      <button class="nav-btn" @click="goZread"> Zread文档
      </button>
    </nav>
    <AppBasic v-if="currentRoute === 'basic'" />
    <AppRandom v-else-if="currentRoute === 'random'" />
    <AppCollab v-else />
  </div>
</template>
<script setup>
import { ref } from "vue";
import AppBasic from "./AppBasic.vue";
import AppRandom from "./AppRandom.vue";
import AppCollab from "./AppCollab.vue";

const currentRoute = ref("basic");
const goZread = () => {
  window.open('https://zread.ai/leepule/vue-canvas-sheet', '_blank')
}
</script>
<style>
* {
  margin: 0;
  padding: 0;
  box-sizing: border-box;
}

body {
  font-family:
    "Inter",
    -apple-system,
    BlinkMacSystemFont,
    "Segoe UI",
    Roboto,
    sans-serif;
  background-color: #f8fafc;
}

.app-root {
  height: 100vh;
  display: flex;
  flex-direction: column;
}

.demo-nav {
  background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%);
  padding: 12px 24px;
  display: flex;
  align-items: center;
  gap: 12px;
  border-bottom: 2px solid #3b82f6;
  flex-shrink: 0;
}

.nav-title {
  color: #94a3b8;
  font-size: 13px;
  font-weight: 500;
}

.nav-btn {
  padding: 8px 20px;
  border: 1px solid #334155;
  background: #1e293b;
  color: #94a3b8;
  border-radius: 6px;
  cursor: pointer;
  transition: all 0.25s ease;
  font-size: 13px;
  font-weight: 500;
}

.nav-btn:hover {
  background: #334155;
  border-color: #475569;
  color: #e2e8f0;
}

.nav-btn.active {
  background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%);
  border-color: #60a5fa;
  color: white;
  box-shadow: 0 4px 12px rgba(59, 130, 246, 0.3);
}

/* GitHub Corner（Tim Holman 经典方案） */
.github-corner {
  position: fixed;
  top: 0;
  right: 0;
  z-index: 1000;
  color: #fff;
}
.github-corner svg {
  display: block;
}
.github-corner .octo-bg {
  fill: #3b82f6;
}
.github-corner:hover .octo-arm {
  animation: octocat-wave 560ms ease-in-out;
}
@keyframes octocat-wave {
  0%, 100% { transform: rotate(0); }
  20%, 60% { transform: rotate(-25deg); }
  40%, 80% { transform: rotate(10deg); }
}
@media (max-width: 500px) {
  /* 小屏幕上 hover 不可靠，改为持续轻微挥动 */
  .github-corner:hover .octo-arm { animation: none; }
  .github-corner .octo-arm { animation: octocat-wave 560ms ease-in-out; }
}
</style>
